...
public class StudentManager : CommonInstance<StudentManager>
{
    ...
    /// <summary>
    /// 测试用：将弟子升级到指定等级（调用正常升级流程）
    /// </summary>
    public void UpgradeStudentToLevelForTest(PeopleData p, int targetLevel)
    {
        if (p == null) return;
        if (targetLevel <= 0) targetLevel = 1;

        int startLevel = p.studentLevel;
        if (startLevel >= targetLevel)
        {
            Debug.Log($"[TestMod] 弟子 {p.name} 当前等级 {startLevel} >= 目标等级 {targetLevel}，无需升级");
            return;
        }

        int maxConfigLevel = DataTable._studentUpgradeList.Count;
        Debug.Log($"[TestMod] 开始将弟子 {p.name} 从等级 {startLevel} 升级到 {targetLevel}，配置表最大等级 {maxConfigLevel}");

        for (int level = startLevel; level < targetLevel; level++)
        {
            if (p.studentLevel > 0 && p.studentLevel <= maxConfigLevel)
            {
                StudentUpgradeSetting setting = DataTable._studentUpgradeList[p.studentLevel - 1];
                int needExp = setting.NeedExp.ToInt32();
                p.studentCurExp += needExp;
        ...
        Debug.Log($"[TestMod] 弟子 {p.name} 升级完成，当前等级 {p.studentLevel}，剩余经验 {p.studentCurExp}");
    }
