...
namespace TooSimpleFramework.UI
{
    ...
    public class OutlineEx : BaseMeshEffect
    {
        ...
        public void ForceSetOutline(int width)
        {
            ...
            Vector4 pUVOrigin)
        {
            ...
            // UV
            var uv = pVertex.uv0;
            uv  = (Vector2)uv+ pUVX / pTriangleX.magnitude * posXOffset * (Vector2.Dot(pTriangleX, Vector2.right) > 0 ? 1 : -1);
            uv  = (Vector2)uv + pUVY / pTriangleY.magnitude * posYOffset * (Vector2.Dot(pTriangleY, Vector2.up) > 0 ? 1 : -1);
            pVertex.uv0 = uv;
            // 原始UV框
